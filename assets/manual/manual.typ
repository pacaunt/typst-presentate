...
The animate function have a `modifier` argument that receives a function that will be called when the element is hidden. In this example, the single bond is hidden by setting its stroke to `gray` and the atoms are hidden by setting their text color to gray.  The animate can process multiple functions at the same time if those functions require the same modifier to be hidden, so `animate` will returns *an array* of _modified_ functions in the same order as specified, as shown when processing `alc.single` and `alc.double` with the same modifier.
